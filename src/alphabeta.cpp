...
#define NINF std::numeric_limits<float>::min();
...
Best_Move *AlphaBetaMax (ChessBoard board, float alpha, float beta, int depth, Move old, int color)
{
	...
	std::vector<Move> Moves = All_Next_Moves(board, player);

	Best_Move *Optimal = new Best_Move;
	for (std::vector<Move>::iterator it = Moves.start(); it != Moves.end(); ++it)
	{
		ChessBoard board_c = *(board.copy());
		Move move = *(it);

		board_c.move(board, move.Old.row, move.Old.col, move.New.row, move.Now.col);

		Best_Move *Last = AlphaBetaMin (board_c, alpha, beta, depth - 1, move, get_opponent_color(color));
...
Best_Move *AlphaBetaMin (ChessBoard board, float alpha, float beta, int depth, Move old, int color)
{
	...
	for (std::vector<Move>::iterator it = Moves.start(); it != Moves.end(); ++it)
	{
		...
		Best_Move *Last = AlphaBetaMax (board_c, alpha, beta, depth - 1, move, get_opponent_color(color));
...
Move AlphaBeta::Generate_Next(ChessBoard board, int depth, int color)
{
	Best_Move *Optimal = AlphaBetaMax(board, NINF, INF, depth, create_move(0, 0, 0, 0), color);
	return Optimal -> move;
}
